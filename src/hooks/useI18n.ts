...
export const useI18nInit = (messages: Messages) => {
  ...
  // first client render
  if (isClient && !i18n.locale) i18n.loadAndActivate({ locale, messages })

  useEffect(() => {
    const localeDidChange = locale !== i18n.locale

    if (localeDidChange) i18n.loadAndActivate({ locale, messages })
  }, [locale, messages])
